...
def construct_conv2d_toeplitz(conv_layer, weight):
    ...
    for i, start_idx in enumerate(corner_indices):
        ...
        #print("kernel_flat:", type(kernel_flat))
        #rows = rows.detach().cpu().numpy() if isinstance(rows, torch.Tensor) else rows
        #cols = cols.detach().cpu().numpy() if isinstance(cols, torch.Tensor) else cols
        rows = ensure_numpy(rows)
        cols = ensure_numpy(cols)
        kernel_flat = ensure_numpy(kernel_flat)
        toeplitz[rows, cols] = kernel_flat            ##############################
        #toeplitz[rows.cpu().numpy().flatten(), cols.cpu().numpy().flatten()] = kernel_flat.cpu().numpy()
    ...
    image_indices = ensure_numpy(image_indices)
    toeplitz = toeplitz.tocsc()[:, image_indices]              ###############################
    #toeplitz = toeplitz.tocsr()[:, image_indices.cpu().numpy().flatten()]
    # Support batching
